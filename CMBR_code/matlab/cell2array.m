...
function Y = cell2array( X, padelement )
    ...
    %make all clips have the same number of frames(if not, the begin frames and end frames are black)
    for d=1:nd inds{d} = 1:maxsiz(d); end;
    Y = X{n}(1);  Y = repmat( Y, ( [ maxsiz, n ] ) );
    for i=1:n 
        pad=(maxsiz(3)-size(X{i},3));
        X{i}=padarray(X{i},[0 0 round(pad/2)],'pre','replicate');
        X{i}=padarray(X{i},[0 0 pad-round(pad/2)],'post','replicate');
        Y( inds{:}, i )=X{i};
    end
