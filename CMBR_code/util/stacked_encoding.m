function [feat] = stacked_encoding( desc,subs, means, covariances, priors,stackGrid)
...
if ~isempty(subs)
    ...
    subs(:,3)=240-a;
    descrsInfo=subs';
    psi = encode_grid(smin(1),smin(2),smin(3), subVolumeSize, desc',stackGrid, descrsInfo, gmm);
    feat=psi;
else
    feat = 1/size(feat,2);
end
