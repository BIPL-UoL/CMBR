...
%% class_train: 1xn
    ...
    for class_ind = 1:num_classes
        ...
        ap = mean(libsvm_cl.sv_coef(Y(libsvm_cl.SVs(:,1)) > 0)) ;
        am = mean(libsvm_cl.sv_coef(Y(libsvm_cl.SVs(:,1)) < 0)) ;
        if ap < am
        ...
        % test it on test
        score_test(:,class_ind) = libsvm_cl.sv_coef' * K_test(libsvm_cl.SVs(:,1),:) - libsvm_cl.rho ;
    end
end