...
function [R,subs,vals,cuboids,h_w,V] = stfeatures( I, sigma, tau, periodic, thresh, maxn, ...
    ...
    h_w=[];
    %%% calculate the window that include the active area of the mouse
%     [start_locs, end_locs,h_w]= activearea_framedifference(I);
%     [subs, vals] = nonmaxsupr_window( subs,vals,start_locs,end_locs,[],maxn); 
%      
    subs_sm = round( imsubs_resize( subs, 1./shrink_all ) );
    
    %%% extract cuboids
    if( nargout<4 && show==0 ) return; end;
    if( length(subs)==0 ) cuboids=uint8([]); return; end;
    cuboids = cell2array( cuboid_extract( I, cuboids_rs, subs, 0 ) ); 
    if( nargout<5 && show==0 ) return; end;
    [V,~,example] = cuboid_display( I_sm, cuboids_rs_sm, subs_sm);
%     imwrite(example,'interest_points.png','png');
%     save('INTEREST_POINTS.mat','V')
    %%% optionally display
    if( show ) 
