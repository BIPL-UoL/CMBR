...
targets=cell(1,length(clips_files));
    ...
    [predicts{1},scores{1},targets{1}] = recog_test_datasets( DATASETS_train,DATASETS_test, 20, 'gmm',[]);
    [predicts{2},scores{2},targets{2}] = recog_test_datasets( DATASETS_test,DATASETS_train, 20, 'gmm',[]);
...
%     end
    ...
    prior=repmat(prior,[1 length(scores{t})]);
    scores{t}=scores{t}./prior;
